...
{
    public class UtilMPipelineSession : IVideoCapture
    {
        ...
        protected byte[] depthFrame1;
        protected byte[] depthFrame2;
        protected byte[] colourFrame1;
        protected byte[] colourFrame2;
        bool phase;

        public byte[] DepthFrame
        {
            get { return phase ? depthFrame1 : depthFrame2; }
        }

        public byte[] ColourFrame
        {
            get { return phase ? colourFrame1 : depthFrame2; }
        }
            ...
            while (capturing)
            {
                ...
                else
                {
                    ...
                    if (captureType == CaptureType.IMAGE_TYPE_DEPTH)
                    {
                        ...
                        if (phase)
                        {
                            depthFrame1 = Helpers.PCXMImageHelper.PXCMImageToByteArray(depthImage, PXCMImage.ColorFormat.COLOR_FORMAT_RGB32, out width, out height);
                        }
                        else
                        {
                            depthFrame2 = Helpers.PCXMImageHelper.PXCMImageToByteArray(depthImage, PXCMImage.ColorFormat.COLOR_FORMAT_RGB32, out width, out height);
                        }
                    }
                    else if (captureType == CaptureType.IMAGE_TYPE_COLOUR)
                    {
                        ...
                        if (phase)
                        {
                            colourFrame1 = Helpers.PCXMImageHelper.PXCMImageToByteArray(colourImage, PXCMImage.ColorFormat.COLOR_FORMAT_RGB32, out width, out height);
                        }
                        else
                        {
                            colourFrame2 = Helpers.PCXMImageHelper.PXCMImageToByteArray(colourImage, PXCMImage.ColorFormat.COLOR_FORMAT_RGB32, out width, out height);
                        }
                    }
